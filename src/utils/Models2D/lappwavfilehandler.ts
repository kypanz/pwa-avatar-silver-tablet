...
export class LAppWavFileHandler {
  ...
  public start(filePath: string): void {
    ...
    if (!this.loadWavFile(filePath)) {
      ...
      // MyAvatarIQ integration 
      // console.log('Finded => ', audio);
      audio.src = filePath;
      // const checkSpeaking = localStorage.getItem('isSpeakingTask');
      // if (checkSpeaking == 'true') {
      //   return console.log('el personaje esta hablando.');
      // }
      audio.play();
      this.is_running = true;
      audio.onended = async () => {
        localStorage.setItem('isSpeakingTask', 'false');
        console.log('se finalizo el mensaje a mencionar');
      }
      // console.log('Current status is_running => ', this.is_running);
      return;
    }
  }
