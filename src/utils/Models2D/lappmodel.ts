...
// import 'whatwg-fetch'; // TEST kypanz
...
import { canvas, ctx, frameBuffer, gl, LAppDelegate } from './lappdelegate';
...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    // TODO : check queue motions
    const audio: any = document.getElementById('voice');
    //console.log('Is voice running ? => ', !audio.paused);
    if (this._motionManager.isFinished() && audio.paused) {
      // console.log('isFinished runned');
      ...
      // console.log('updateMotion runned')
    ...
    if (this._lipsync) {
      ...
      value = this._wavFileHandler.getRms() * 15;
    ...
    //const audio: any = document.getElementById('voice');
    //console.log('output .wav file => ', audio); // Output => testing.wav
  ...
  public startMotion(
    ...
    // NOTA : Esto se repito en bucle
    // - PRUEBA LUEGO BORRAR 
    //
    const audio_url_blob = localStorage.getItem('ky-to-say');
    if (!audio_url_blob) {
      return console.error('no se detectó el audio a mencionar');
    }

    // Obtenemos el último audio reproducido
    const lastSaid = localStorage.getItem('last-said');

    // Si es diferente al anterior, lo reproducimos
    if (lastSaid !== audio_url_blob) {
      this._wavFileHandler.start(audio_url_blob);
      localStorage.setItem('last-said', audio_url_blob);
    }
    //
    // - END PRUEBA
    ...
    // TODO : voice handler here | boca | mouth | kypanz test
    //voice
    ...
    if (voice.localeCompare('') != 0) {
      ...
      const audio_url_blob = localStorage.getItem('ky-to-say');
      if (!audio_url_blob) return console.error('no se detecto el audio a mencionar');
      this._wavFileHandler.start(audio_url_blob);
      // this._wavFileHandler.start('https://cdn.freesound.org/previews/795/795971_5674468-lq.mp3');
    }
   ...
   */
  // This is gonna be selected on click
  ...
  public constructor() {
    ...
    // console.log('test => CubismFramework => ', CubismFramework.getIdManager());
    ...
    // Setting global for wavFileHandler | kypanz test
    (window as any).wavFileHandler = this._wavFileHandler;
